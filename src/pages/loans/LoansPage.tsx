...
const LoansPage: React.FC = () => {
  ...
  const cargarSolicitudes = async () => {
    ...
    try {
      ...
      const aprobadas = response.solicitudes.filter(s => 
        s.estado === 'APROBADA' || 
        s.estado === 'DOCUMENTACION_VALIDADA' || 
        s.estado === 'CONTRATO_VALIDADO'
      ).length;
      const rechazadas = response.solicitudes.filter(s => 
        s.estado === 'RECHAZADA' || 
        s.estado === 'CONTRATO_RECHAZADO'
      ).length;
      const montoTotal = response.solicitudes.reduce((sum, s) => sum + s.montoSolicitado, 0);
      
      // Logs para debug
      console.log('Estados de solicitudes:', response.solicitudes.map(s => ({ numero: s.numeroSolicitud, estado: s.estado })));
      console.log('Métricas calculadas:', { totalSolicitudes, enBorrador, aprobadas, rechazadas, montoTotal });
